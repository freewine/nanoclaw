...
describe('mimo-v2-tts skill package', () => {
  ...
  it('has a valid manifest', () => {
    ...
    expect(content).toContain('skill: mimo-v2-tts');
    expect(content).toContain('version: 1.0.0');
    expect(content).toContain('MIMO_API_KEY');
  });
  ...
  it('has all files declared in adds', () => {
    ...
    expect(content).toContain('mimo-v2-tts');
    expect(content).toContain('xiaomimimo.com');
  });
  ...
  it('modified ipc-mcp-stdio.ts adds send_tts_message tool with MiMo voices', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'modify', 'container', 'agent-runner', 'src', 'ipc-mcp-stdio.ts'),
      'utf-8',
    );

    // TTS tool with MiMo voices
    expect(content).toContain('send_tts_message');
    expect(content).toContain("type: 'tts_message'");
    expect(content).toContain('voice');
    expect(content).toContain('MiMo');
    expect(content).toContain('mimo_default');
    expect(content).toContain('default_zh');
    expect(content).toContain('default_en');
  ...
  it('add/src/tts.ts validates voice names and uses MiMo API', () => {
    const content = fs.readFileSync(
