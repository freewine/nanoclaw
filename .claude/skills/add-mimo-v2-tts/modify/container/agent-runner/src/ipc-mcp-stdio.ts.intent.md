...
## What changed
Added `send_tts_message` MCP tool that sends text to the server for TTS generation via MiMo v2. The server handles audio generation — the agent just provides text and optional voice name.
...
- Added: `send_tts_message` tool with `text` and optional `voice` parameters
  ...
  - Tool description includes speech formatting tips and available MiMo voices (mimo_default, default_zh, default_en)
